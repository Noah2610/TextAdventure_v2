...
## Game Information
### Settings
There is a YAML configuration file at `./src/settings.yml` which contains some general settings for the game.  
...
```
Events are basically Room specific Verbs.  
They don't have configuration files; their keywords with which to use them in-game  
are defined in the config file of the Room it is supposed to be usable in.  
Every Event has their own class with an `action` method  
which is called when the player uses the Event.
