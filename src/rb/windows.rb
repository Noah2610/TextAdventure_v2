...
### Here the main Curses Windows used in the game are defined
##  ex.: Input Window, Main Output Window, Conversation Output Window, ...

### Define valid input keys
#VALID_KEYS = [
#	(65 .. 90) .to_a.map(&:chr),  # uppercase leters
#	(97 .. 122).to_a.map(&:chr),  # lowercase leters
#	(32 .. 64) .to_a.map(&:chr),  # digits, space, punctuation, misc symbols
#].flatten
VALID_KEYS = [
	(65 .. 90) .to_a,
	(97 .. 122).to_a,
